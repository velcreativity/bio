#!/usr/bin/env python3
"""CUT06-09 엘리베이터 씬 - 붙여넣기용 .txt 생성기.

GLOBAL / LOCKS는 전 컷 공유. 여기서 한 번 고치고 다시 실행하면
개별 파일과 통합 파일이 전부 갱신된다.

    python3 scenario/prompts/build_txt.py
"""
...
GLOBAL = """Format: 16:9 widescreen, 24 fps, 180-degree shutter motion blur. Photorealistic live action - no 3D render, no game engine.
...
LOCKS = """The set contains only what the reference frame shows - no added corridors, doors, rooms, furniture or geography beyond the reference. Architecture stays fixed in place for the whole clip; walls, seams, handrails and floor hold their exact positions.
...
CUTS = [
    {
        "file": "CUT06_corridor_approach.txt",
        "body": """SCENE CONTEXT
...
24 fps, 180-degree shutter motion blur, physical cine lens, photorealistic live action.""",
        "locks": """The elevator doors stay fully closed for the entire clip. She walks away from the lens for the whole clip and never turns back toward it.""",
    },
    {
        "file": "CUT07_call_button_doors_open.txt",
        "body": """SCENE CONTEXT
...
STYLE
24 fps, 180-degree shutter motion blur, physical cine lens, photorealistic live action.""",
        "locks": """The call lamp is dark until the click and stays lit afterwards. The doors are closed at the first frame, open once, and stay open for the rest of the clip. The camera holds locked off and still.""",
    },
    {
        "file": "CUT08_select_B1_doors_close.txt",
        "body": """SCENE CONTEXT
...
STYLE
24 fps, 180-degree shutter motion blur, physical cine lens, photorealistic live action.""",
        "locks": """The doors stay fully closed for the entire clip. The indicator display and every button legend hold exactly the characters the reference shows and do not change. The B1 legend stays lit amber throughout. The camera stays locked and the only movement in frame is vibration.""",
    },
]

STILLS = [
    (
        "STILL_CUT07_FIRST_FRAME.txt",
        "Photorealistic still frame, 16:9. A figure in a white belted lab coat with dark hair "
        "gathered in a low bun stands with her back to the lens, an arm's length from a pair of "
        "closed brushed-steel elevator doors at the end of a narrow steel-clad corridor. She holds "
        "one red apple in her right hand at hip height. Her face is not visible. A call plate with a "
        "single round button sits on the wall to the right of the doors at 1.1 m, its lamp unlit. "
        "Steel wall panels with vertical seams run left and right; recessed linear fluorescents at "
        "4000K run overhead. Warm-gray floor. Cold desaturated institutional palette, gunmetal and "
        "brushed steel, the apple the only saturated colour. Sharp focus throughout, deep depth of "
        "field, natural skin texture, subtle imperfections. Practical ceiling light only, no "
        "daylight, exposure 0.3 stop under key.",
    ),
    (
        "STILL_CUT08_FIRST_FRAME.txt",
        "Photorealistic still frame, 16:9. Interior of an elevator car photographed from standing "
        "eye height at 1.55 m in the middle of the car, facing a pair of open brushed-steel doors "
        "with a dim steel corridor visible beyond. Gunmetal wall panels left and right, one "
        "brushed-steel handrail along each at 0.9 m. A control panel on the right wall runs as a "
        "single vertical column - an indicator display at the top, a round call icon below it, then "
        "four square buttons reading 2, 1, B1, B2 downward, then two door-control buttons. Every "
        "button legend is unlit. One linear ceiling fixture at 4000K above the door header throws a "
        "bright band across it. Textured warm-gray floor. Cold desaturated institutional palette. "
        "Sharp focus throughout, deep depth of field, brushed metal with micro-scratches and uneven "
        "specular. Practical ceiling light only, no daylight, exposure 0.3 stop under key.",
    ),
]

SETTINGS = """[UI SETTINGS]  * 프롬프트 본문에 넣지 말 것
Model            Seedance 2.0
Aspect ratio     16:9
...
start_image      CUT06 = 복도 뒷모습 플레이트
                 CUT07 = STILL_CUT07_FIRST_FRAME 으로 생성
                 CUT08 = STILL_CUT08_FIRST_FRAME 으로 생성
                 CUT09 = 차내 플레이트 (B1 점등)
end_image        CUT08 = 차내 플레이트 (B1 점등)  <- 이 컷만 end_image 사용

[편집]
컷 순서 CUT06 > CUT07 > CUT08 > CUT09
CUT07 / CUT08 / CUT09 는 4.0초 전체 사용 (페이오프가 3초 이후에 있음)
CUT06 만 3.0초에서 잘라도 됨
길면 CUT09 를 먼저 버린다 - CUT08 이 문 닫힘으로 끝나므로 씬은 성립함"""
...
def main():
    ...
    for cut in CUTS:
        path = OUT / cut["file"]
        path.write_text(assemble(cut), encoding="utf-8")
        written.append(path)
    ...
    parts = [SETTINGS]
    for cut in CUTS:
        label = cut["file"].replace(".txt", "")
        parts.append(f"{bar}\n{label}   <- 아래 한 덩어리를 통째로 복사\n{bar}\n\n{assemble(cut)}")
    for name, text in STILLS:
        label = name.replace(".txt", "")
        parts.append(f"{bar}\n{label}   <- 스틸 이미지 생성용\n{bar}\n\n{text}\n")

    path = OUT / "ALL_CUT06-09.txt"
    path.write_text("\n\n".join(parts), encoding="utf-8")
    written.append(path)

    for p in written:
        print(f"{p.stat().st_size:>7,} B  {p.relative_to(OUT.parent)}")
